...
(function(){
  ...
  document.querySelectorAll('[data-download-target]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const el=document.getElementById(btn.dataset.downloadTarget); if(!el) return;
      const blob=new Blob([el.textContent],{type:btn.dataset.mime||'text/plain;charset=utf-8'});
      const url=URL.createObjectURL(blob); const a=document.createElement('a');
      a.href=url; a.download=btn.dataset.filename||'source.txt'; document.body.appendChild(a); a.click(); a.remove();
      setTimeout(()=>URL.revokeObjectURL(url),1000);
    });
  });
  document.querySelectorAll('[data-card-search]').forEach(search=>{
    const scope=search.closest('.section-body') || document;
    search.addEventListener('input',()=>{ const q=search.value.toLowerCase(); scope.querySelectorAll('[data-search-card]').forEach(c=>{ c.style.display=c.textContent.toLowerCase().includes(q)?'block':'none'; }); });
  });
})();
